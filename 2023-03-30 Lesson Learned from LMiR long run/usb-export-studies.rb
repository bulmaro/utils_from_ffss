...
class UsbExportStudies < SonoSiteSystem::Test
  ...
  def initialize(script_name)
    ...
    @lmir_supported = !@test_info.test_inputs[:lmir_supported].nil? && @test_info.test_inputs[:lmir_supported]
  end
  ...
  def execute
    show_log 'LMiR feature enabled' if @lmir_supported
    ...
    @num_exports.times do |iteration|
      ...
      while clinical_display.icons.names.any?{|icon_name| USB_EXPORT_ICONS.include?(icon_name)}
        ...
        if @lmir_supported
          ...
          test_option = views_predefined.sample
        end
